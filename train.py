...
def train(config: TrainingConfig):
    ...
    if config.freeze_backbone_epochs > 0:
        (model.module if isinstance(model, nn.DataParallel) else model).freeze_backbone()
    ...
    # ── Optimizer ─────────────────────────────────────────────────────────────
    optimizer = (
        torch.optim.AdamW(model.parameters(), lr=config.learning_rate, weight_decay=config.weight_decay)
        if config.optimizer.lower() == "adamw"
        else torch.optim.SGD(model.parameters(), lr=config.learning_rate, momentum=0.9, weight_decay=config.weight_decay)
    )
    ...
    ckpt_manager = CheckpointManager(config.checkpoint_dir, metric=config.metric_for_best, keep_top_k=config.save_top_k)
    ...
    for epoch in range(start_epoch, config.num_epochs + 1):
        ...
        train_metrics = train_one_epoch(
            ...
            device, scaler, epoch, config, amp_device=amp_device,
        )

        # Validation
        val_metrics = {}
        if val_loader is not None and epoch % config.eval_every_n_epochs == 0:
            val_metrics = validate(model, val_loader, loss_fn, device, config, amp_device=amp_device)
            ...
            ckpt_manager.save_checkpoint(
                model=inner, optimizer=optimizer, epoch=epoch,
                metrics=val_metrics, config=config.__dict__,
                is_best=is_best, scheduler=scheduler, scaler=scaler,
            )
            ...
            if is_best:
                ...
                if config.early_stopping and no_improve >= config.patience:
                    logger.info(f"Early stopping at epoch {epoch} ({no_improve} epochs without improvement)")
                    break
        else:
            logger.info(
                f"Epoch {epoch}/{config.num_epochs}  "
...
def parse_args():
    ...
    p.add_argument("--train_dir",      type=str, required=True, help="Root dir with MAP1, MAP2, … folders")
    p.add_argument("--val_dir",        type=str, default=None,  help="Validation dir (optional; else auto-split)")
    p.add_argument("--batch_size",     type=int, default=8)
    p.add_argument("--epochs",         type=int, default=100)
    p.add_argument("--lr",             type=float, default=2e-4)
    p.add_argument("--image_size",     type=int, default=512)
    p.add_argument("--backbone",       type=str, default="resnet50", choices=["resnet50", "resnet34"])
    p.add_argument("--num_workers",    type=int, default=0, help="0 = main process (required for Jupyter/shared servers)")
    p.add_argument("--checkpoint_dir", type=str, default="checkpoints")
    p.add_argument("--name",           type=str, default="baseline")
    p.add_argument("--quick_test",     action="store_true", help="Quick 5-epoch smoke-test with resnet34")
    return p.parse_args()
