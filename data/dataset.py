...
SHAPEFILE_TASKS = [
    ...
    (["Railway*.shp", "Rail*.shp", "Railway.shp"], ["railway", "railroad", "rail"], "railway"),
]
...
class SvamitvaDataset(Dataset):
    ...
    def _compute_tiles(self, tif_path: Path):
        """Open the TIF, compute tile windows. Returns tuple or None on error."""
        try:
            ...
            for patterns, keywords, task_key in SHAPEFILE_TASKS:
                ...
                if shp:
                    ...
                    if task_key in OPTIONAL_TASKS:
                        ...
                        logger.warning(f"[{map_dir.name}] No shapefile for '{task_key}'")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        elif image.dtype == np.uint16: image = image.astype(np.float32) / 65535.0
        else:                          image = image.astype(np.float32)
        ...
        for _, _, task_key in SHAPEFILE_TASKS:
            ...
            if task_key in sample["annotations"]:
                gdf = self._get_cached_gdf(
                    sample["map_name"], task_key,
                    sample["annotations"][task_key], tif_crs,
                )
        ...
        if "building" in sample["annotations"]:
            bgdf = self._get_cached_gdf(
                sample["map_name"], "building",
                sample["annotations"]["building"], tif_crs,
            )
...
def create_dataloaders(
    ...
    """
    train_ds = SvamitvaDataset(train_dir, image_size, get_train_transforms(image_size), "train")

    if val_dir is not None:
        val_ds_full = SvamitvaDataset(val_dir, image_size, get_val_transforms(image_size), "val")
        tr_ds  = train_ds
        val_ds = val_ds_full
